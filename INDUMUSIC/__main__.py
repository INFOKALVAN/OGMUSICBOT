...
import config
from INDUMUSIC import LOGGER, app, userbot
from INDUMUSIC.core.call import INDU
from INDUMUSIC.misc import sudo
from INDUMUSIC.plugins import ALL_MODULES
from INDUMUSIC.utils.database import get_banned_users, get_gbanned
from INDUMUSIC.utils.cookie_handler import fetch_and_store_cookies
...
async def init():
    ...
    try:
        ...
        LOGGER("INDUMUSIC").info("ʏᴏᴜᴛᴜʙᴇ ᴄᴏᴏᴋɪᴇs ʟᴏᴀᴅᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ ✅")
    except Exception as e:
        LOGGER("INDUMUSIC").warning(f"⚠️ ᴄᴏᴏᴋɪᴇ ᴇʀʀᴏʀ: {e}")
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("INDUMUSIC.plugins" + all_module)

    LOGGER("INDUMUSIC.plugins").info("ᴍᴏᴅᴜʟᴇs ʟᴏᴀᴅᴇᴅ...")

    await userbot.start()
    await INDU.start()

    try:
        await INDU.stream_call("https://files.catbox.moe/euj0oc.mp4")
    except NoActiveGroupCall:
        LOGGER("INDUMUSIC").error(
    ...
    await INDU.decorators()
    LOGGER("INDUMUSIC").info("✅ Indu music Bot Started Successfully!")
    await idle()
    await app.stop()
    await userbot.stop()
    LOGGER("INDUMUSIC").info("sᴛᴏᴘᴘɪɴɢ ᴍᴜsɪᴄ ʙᴏᴛ ...")
            ...
            try:
                ...
                if _sync_db is not None:
                    try:
                        ...
                        LOGGER("INDUMUSIC").warning(f"DB stats error: {db_error}")

                return f"""<!DOCTYPE html>
<html><head><title>Indu Music Bot</title>
...
<div class="h"><h1>🎵 Indu Music Bot</h1><div class="sb"><span class="dot"></span><span style="color:#00ff88;font-size:.9em">Online & Running</span></div></div>
<div class="sg">
...
<div class="f"><p>Auto-refreshes every 30s</p><p style="margin-top:8px"><a href="https://t.me/{config.BOT_USERNAME}">Open Bot</a> • <a href="https://t.me/ItsMeInduBots">Developer</a></p></div>
</div></body></html>"""
            except Exception as e:
                LOGGER("INDUMUSIC").error(f"Flask home error: {e}")
        ...
        def api_stats():
            ...
            try:
                ...
                LOGGER("INDUMUSIC").error(f"Flask API stats error: {e}")
