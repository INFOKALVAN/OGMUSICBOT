...
from INDUMUSIC.mongo.welcomedb import is_on, bump, cool, auto_on
from INDUMUSIC.utils.colored_buttons import styled_button, buttons_to_inline_markup

BG_PATH = "INDUMUSIC/assets/INDU/welcome.png"
FALLBACK_PIC = "INDUMUSIC/assets/upic.png"
FONT_PATH = "INDUMUSIC/assets/INDU/Arimo.ttf"
...
def build_pic(av, fn, uid, un):
    ...
    # 'INDUMUSIC/assets/indu/welcome.png' file.
...
async def welcome(client, update: ChatMemberUpdated):
    ...
    try:
        ...
        for f in (avatar, img):
            if f and os.path.exists(f) and "INDUMUSIC/assets" not in f:
                try:
                    os.remove(f)
                except:
                    pass
