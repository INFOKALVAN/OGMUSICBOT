...
import config
from INDUMUSIC import Carbon, YouTube, app
...
from INDUMUSIC.utils.database import add_active_video_chat, is_active_chat
from INDUMUSIC.utils.exceptions import AssistantErr
from INDUMUSIC.utils.inline import aq_markup, close_markup
...
from INDUMUSIC.utils.stream.queue import put_queue, put_queue_index
...
async def stream(
    ...
    if forceplay:
        await INDU.force_stop_stream(chat_id)
    ...
    if streamtype == "playlist":
        ...
        for details in details_list:
            ...
            if await is_active_chat(chat_id):
                ...
                from INDUMUSIC.utils.inline.play import colored_stream_markup
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await INDU.join_call(chat_id, original_chat_id, file_path, video=False)
            if not await is_active_chat(chat_id):
                raise AssistantErr(_["call_6"])
            await put_queue(
                chat_id,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await INDU.join_call(chat_id, original_chat_id, file_path, video=is_video)
            if not await is_active_chat(chat_id):
                raise AssistantErr(_["call_6"])
            await put_queue(
                chat_id,
